...
function loadQuestion() {
  ...
  for (let i = 0; i < item.options.length; i++) {
    const answerOptions = $('<button>');
    answerOptions.addClass('guess').html(item.options[i]).val(i);
  ...
  $('.guess').on('click', function () {
    ...
    if (guess === item.answer) {
      ...
      wrong++;
      guess = '';
      $('#answer').html(`<p>Wrong! The correct answer is: ${item.options[item.answer]}</p>`).append(`<img src=${item.image}>`);
      showScores();
    }
  });
}
...
function setTimer() {
  ...
  if (timer < 0) {
    stopTimer();
    unanswer++;
    $('#answer').html(`<p>Time is up! The correct answer is: ${item.options[item.answer]}</p>`).append(`<img src=${item.image}>`);
    showScores();
  }
}
...
function showScores() {
  setTimeout(() => {
    ...
    if (wrong + correct + unanswer === trivia.length) {
      ...
      $('#answer').append(`<h4> Correct: ${correct}</h4>`, `<h4> Incorrect: ${wrong}</h4>`, `<h4> Unanswered: ${unanswer}</h4>`);
      $('#replay').show();
      correct = 0;
      wrong = 0;
      unanswer = 0;
